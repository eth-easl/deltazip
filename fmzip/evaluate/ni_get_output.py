...
def main(args):
    ...
    tokenizer = transformers.AutoTokenizer.from_pretrained(
        args.base_model, use_fast=True, padding_side="left"
    )
    text_generation_pipeline = transformers.TextGenerationPipeline(
        model=base_model, tokenizer=tokenizer, batch_size=8, device="cuda:0"
    )
    output = []
    with open(f".cache/ni_calib/test_references/{args.task}.jsonl", "r") as fp:
        ...
        with open(
            f'.cache/eval/{args.task}/{args.base_model.replace("/",".")}.jsonl', "w"
        ) as fp:
            for line in output:
