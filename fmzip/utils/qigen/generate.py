    ...
    try:
        ...
        try:
            ...
            if gs != -1:
                ...
                if verbose:
                    print(
                        "Failed to solve, using heuristic. mb = ", mymb, "tb = ", mytb
                    )
...
def gen_and_compile(
    n, m, t, nb, mb, tb, nu, mu, tu, p, unroll, bits=4, gs=-1, module=False
):
    split = np.ones(p)
    split = split * tb
    while np.sum(split) < t:
    ...
    if module:
        ...
        code += print_parameters(
            bits, n, m, t, nb, mb, tb, mu, nu, tu, unroll, p, gs=gs
        )
...
def forward_module_gs(nu, mu, tu, p, unroll, bits):
    ...
    for i in range(nu):
        for j in range(0, tu, 8):
            store += (
                f"__m256 f{i}_{j} = _mm256_fmadd_ps(acc{i}_{j}, s{i}_{j}, o{i}_{j});\n"
            )
...
tid*tt*m/{packed}*3;\n"
    ...
    for i in range(0, tu, 8):
        ...
        res += (
            f"__m256 z{i} = _mm256_loadu_ps(&zeros[base_output + i1* t + j + {i}]);\n"
        )
    # for i in range(0,tu,8):
...
def unpack_zeros(bits):
    ...
    res += (
        f"void unpack_zeros{bits}(torch::Tensor zeros, torch::Tensor out, int N, int M)"
    )
...
def gen_module_search(r, p, bits_list=[2, 3, 4]):
    ...
    for bits in bits_list:
        ...
        code += qforward(
            nu_gs, mu_gs, tu_gs, p, unroll_gs, bits=bits, module=True, gs=True
        )
