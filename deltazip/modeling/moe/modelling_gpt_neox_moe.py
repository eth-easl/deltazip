...
class GPTNeoXMLP(nn.Module):
    ...
    def forward(self, hidden_states):
        # self.dense_h_to_4h = self.dense_h_to_4h.to(hidden_states.device)
        # self.dense_4h_to_h = self.dense_4h_to_h.to(hidden_states.device)
...
class MoE(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.mlp = nn.ModuleList([GPTNeoXMLP(config) for _ in range(config.num_experts)])
    ...
    def forward(self, x):
        ...
        scores = self.gate(x)
        expert_weights, expert_indices = torch.topk(scores, self.experts_per_token, dim=-1)
...
class GPTNeoXLayer(nn.Module):
    ...
    def forward(
        ...
        if self.use_parallel_residual:
            ...
            moe_output = self.post_mlp_dropout(moe_output)
            hidden_states = moe_output + attn_output + hidden_states
        else:
            # pseudocode:
            # x = x + attn(ln1(x))
            # x = x + mlp(ln2(x))
            attn_output = attn_output + hidden_states
            moe_output = self.moe(self.post_attention_layernorm(attn_output))
            moe_output = self.post_mlp_dropout(moe_output)
            hidden_states = moe_output + attn_output
