...
def test_allocate():
    ...
    block_manager = BlockSpaceManager(
        block_size, num_cpu_blocks, num_gpu_blocks, watermark=0
    )
    ...
    block_manager = BlockSpaceManager(
        block_size, num_cpu_blocks, num_gpu_blocks, watermark=1 / num_gpu_blocks
    )
...
def test_append_slot_cow():
    ...
    seq_group = SequenceGroup(
        "1", [prompt, child], SamplingParams(), time.time(), time.perf_counter
    )
    block_manager.allocate(seq_group)
...
def test_fork():
    ...
    prompt, seq_group = create_dummy_prompt("1", block_size - 1, block_size=block_size)
    block_manager.allocate(seq_group)

    # Fork prompt and copy block tables.
    child = prompt.fork(2)
    block_manager.fork(prompt, child)
    assert block_manager.get_block_table(prompt) == block_manager.get_block_table(child)
    token_id = 4
...
def test_sliding_window_multi_seq():
    ...
    seq_group = SequenceGroup("1", [parent], SamplingParams(), time.time(), None)
    block_manager.allocate(seq_group)
    ...
    assert block_manager.get_num_free_gpu_blocks() == num_gpu_blocks - sliding_window
    ...
    assert block_manager.get_block_table(parent) == block_manager.get_block_table(child)
    ...
    assert (
        block_manager.get_num_free_gpu_blocks() == num_gpu_blocks - sliding_window - 1
    )
