...
def test_sample_decoding_only(
    random_sampling, max_best_of, modify_greedy_probs, seed, vocab_size, save_logprobs
):
    ...
    if random_sampling == "mixed":
        random_sampling_mask = (torch.rand((1, bs), device="cuda") < 0.5).expand(
            n_splits, bs
        )
    elif random_sampling:
        random_sampling_mask = torch.ones(
            (n_splits, bs), dtype=torch.bool, device="cuda"
        )
    else:
        random_sampling_mask = torch.zeros(
            (n_splits, bs), dtype=torch.bool, device="cuda"
        )

    seeds = torch.randint(
        1, torch.iinfo(torch.long).max, (n_splits, bs), device="cuda"
    ).mul_(random_sampling_mask)
    ...
    for i in range(bs):
        ...
        if modify_greedy_probs and not request_uses_random_sampling:
            ...
            assert not torch.allclose(
                sampled_modified_probs[i][0], probs[i][sampled_tokens[i]]
            )
    ...
    if save_logprobs:
        ...
        for i in range(bs):
            for best_of in range(max_best_of):
                assert torch.all(
                    sampled_logprobs[i] == logprobs[i][sampled_tokens[i, best_of]]
                )
    else:
...
def test_sample_prompt_logprobs(
    random_sampling, max_best_of, modify_greedy_probs, seed, vocab_size
):
    set_random_seed(seed)
    ...
    sample_indices = torch.tensor(
        prompt_sizes, dtype=torch.long, device="cuda"
    ).cumsum_(0)
    n_splits = get_num_triton_sampler_splits(probs.shape[1])
    if random_sampling == "mixed":
        random_sampling_mask = torch.rand((n_splits, samples), device="cuda") < 0.5
    elif random_sampling:
        random_sampling_mask = torch.ones(
            (n_splits, samples), dtype=torch.bool, device="cuda"
        )
    else:
        random_sampling_mask = torch.zeros(
            (n_splits, samples), dtype=torch.bool, device="cuda"
    ...
    seeds = torch.randint(
        1, torch.iinfo(torch.long).max, (n_splits, samples), device="cuda"
    ).mul_(random_sampling_mask)
    ...
    for i, t in enumerate(sample_indices):
        ...
        for best_of in range(max_best_of):
            assert torch.all(
                sampled_logprobs[i]
                == logprobs[sample_indices[i]][sampled_tokens[i, best_of]]
            )
...
def test_get_sequence_seeds(seed):
    ...
    for i in range(512):
        new_seq_seed = SamplingTensors._get_sequence_seeds(
            starting_seed, i, seeds_to_generate=1, is_greedy=False
        )[0]
        new_seq_seed_extra_entropy = SamplingTensors._get_sequence_seeds(
            starting_seed, i, extra_entropy, seeds_to_generate=1, is_greedy=False
        )[0]
