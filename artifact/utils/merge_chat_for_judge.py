...
with open("artifact/results/lmsys_output_xwin_16bit.jsonl", "r") as fp:
    full_data = [json.loads(line) for line in fp][:1000]

with open("artifact/results/lmsys_output_xwin_2bit.jsonl", "r") as fp:
...
with open("artifact/results/xwin_agg.jsonl", "w") as fp:
    for d in data:
