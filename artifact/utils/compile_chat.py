...
def render_dialog(question, uncompressed_response, compressed_response):
    ...
    if (
        any([ord(c) >= 128 for c in question])
        or any([ord(c) >= 128 for c in uncompressed_response])
        or any([ord(c) >= 128 for c in compressed_response])
    ):
        return None
    # check if contains latex special characters
    if (
        any([c in ["&", "%", "$", "#", "_", "{", "}"] for c in question])
        or any(
            [c in ["&", "%", "$", "#", "_", "{", "}"] for c in uncompressed_response]
        )
        or any([c in ["&", "%", "$", "#", "_", "{", "}"] for c in compressed_response])
    ):
        return None
    # check if contains "\"
    if (
        any([c == "\\" for c in question])
        or any([c == "\\" for c in uncompressed_response])
        or any([c == "\\" for c in compressed_response])
    ):
        return None
    # remove lyrics
    if (
        "Lyrics" in question
        or "Lyrics" in uncompressed_response
        or "Lyrics" in compressed_response
    ):
        return None
    template = """
    \dia[User]{<question>}
...
with open("artifact/results/quality/chat/vicuna.jsonl", "r") as fp:
    ...
    for datum in data:
        ...
        uncompressed_response = [
            answer for answer in answers if answer["from"] == "original"
        ][0]["text"]
        compressed_response = [
            answer for answer in answers if answer["from"] == "fmzip"
        ][0]["text"]
        dialogs.append(
            render_dialog(question, uncompressed_response, compressed_response)
        )
