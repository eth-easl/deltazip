...
def plot(args):
            ...
            cdf_df.append(
                {
                    "provider": provider,
                    "t": t,
                    "cdf": provider_df[provider_df["time_elapsed"] <= t].shape[0]
                    / provider_df.shape[0],
                }
            )
    ...
    fig.update_layout(
        ...
        height=600,
        title_x=0.5,
        title_text="Cumulative Distribution Function of Latency",
    )
    fig.update_layout(
        font_family="Arial",
        font_color="black",
    ...
    fig.update_layout(
        yaxis=dict(title_text="CDF", title_font=dict(size=22), tickfont_size=18)
    )
    fig.update_layout(
        xaxis=dict(title_text="Latency (s)", title_font=dict(size=22), tickfont_size=18)
    )
