...
def get_provider_name(provider):
    if provider["name"] == "hf":
        ...
        return f"{project_name}<br>bsz={provider['args'].get('batch_size', 1)}<br>{strategy_mapping[provider['args'].get('placement_strategy','none')]}, {'Lossless' if provider['args'].get('lossless_only', False) else 'Lossy'}"
