...
def plot(args):
    ...
    fig.update_layout(
        ...
        height=600,
        title_x=0.5,
        title_text=f"Request ID vs. Latency of Different Backends ({num_tokens} Tokens)",
    )
    fig.update_layout(
        font_family="Arial",
        font_color="black",
    ...
    fig.update_layout(
        yaxis=dict(title_text="Latency", title_font=dict(size=22), tickfont_size=18)
    )
    fig.update_layout(
        xaxis=dict(title_text="Request ID", title_font=dict(size=22), tickfont_size=18)
    )
