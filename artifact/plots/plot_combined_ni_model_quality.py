...
def plot(args):
    ...
    for mid, model in enumerate(base_models):
        ...
        sub_df['task_name'] = sub_df['task'].apply(lambda x: x.split('_')[0].replace('task','NI-'))
        sub_df = sub_df[~sub_df['task_name'].isin(remove_tasks)]
        sub_df['method'] = sub_df['method'].apply(lambda x: project_name_mapping[x])
        fig2 = px.line(sub_df, y="eval_res", x="compression ratio", color="task_name", line_dash="method", markers=True,)
        for fig2_data in fig2['data']:
            fig.add_trace(
                go.Scatter(
                    ...
                    y=fig2_data['y'],
                    name=fig2_data['name'],
                    line=dict(dash='solid' if 'FiniCompress' in fig2_data['name'] else 'dash', color=fig2_data['line']['color']),
                    mode='lines+markers',
