...
for batch_size in range(1, BATCH_SIZE+1):
    ...
    for iter in range(5):
        ...
        for i in range(batch_size):
            res = quant_matmul_248(input[i], qweights[i], scales[i], qzeros[i], g_idx[i], BITS, MAX_Q)
            results.append(res)
        ...
        batch_results = quant_bmm_248(input, b_qweights, b_scales, b_qzeros, b_g_idx, BITS, MAX_Q)
        torch.cuda.synchronize()
        end = timer()
        if iter > 0:
            timing_results.append({"batch": end - start,"loop": loop_time, "batch_size": batch_size})
