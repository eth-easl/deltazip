...
for task in tasks:
    ...
    for step in steps:
        if not os.path.exists(os.path.join(out_dir, task, step, "config.json")):
            job = f"python cli/compress.py --target-model {os.path.join(in_folder, task, step)} --outdir {os.path.join(out_dir, task, step)} --dataset {os.path.join(ar_dataset, task+'.train.jsonl')} --n-samples 256 --bits 3 --group-size 128 --sparsity 0.75 --lossless gdeflate --delta subtract --base-model openlm-research/open_llama_3b_v2"
            jobs.append(job)
